...
const feature = loadFeature("./__tests__/features/Timeclock-scenario.feature");
...
defineFeature(feature, (test) => {
  ...
  test("User navigates to Timeclock", ({ given, when, then }) => {
    let timeClockBloack: ShallowWrapper;
    let instance: Timeclock;

    given("I am a User loading Timeclock", () => {
      timeClockBloack = shallow(<Timeclock {...screenProps} />);
    });

    when("I navigate to the Timeclock", () => {
      instance = timeClockBloack.instance() as Timeclock;
    ...
    then("Timeclock will load with out errors", () => {
      expect(timeClockBloack).toBeTruthy();
    });
    ...
    then("I can press start game button with out errors", () => {
      let testStartGameButton = timeClockBloack.findWhere(
        (node) => node.prop("testID") === "testStartGameButton"
      );
      testStartGameButton.simulate("press");
    ...
    then("I can press finish game button with out errors", () => {
      let testFinishGameButton = timeClockBloack.findWhere(
        (node) => node.prop("testID") === "testFinishGameButton"
      );
      testFinishGameButton.simulate("press");
    ...
    then("I can press start game button with errors", () => {
      ...
      expect(instance.state.dataLoading).toEqual(false);
    });
    ...
    then("I can press start game button with 502 errors", () => {
      ...
      expect(instance.state.dataLoading).toEqual(true);
    });
