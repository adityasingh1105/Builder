...
defineFeature(feature, (test) => {
  ...
  test("User navigates to FileAttachment", ({ given, when, then }) => {
    ...
    then("FileAttachment will load with out errors", () => {
      ...
      component
        .findWhere((node) => node.prop("data-test-id") === "deleteButton")
        .simulate("click");
      expect(instance.deleteFile).toHaveBeenCalled();

      component
        .findWhere((node) => node.prop("data-test-id") === "viewButton")
        .simulate("click");

      component
        .findWhere((node) => node.prop("data-test-id") === "downloadButton")
        .simulate("click");

      component
        .findWhere((node) => node.prop("data-test-id") === "editButton")
        .simulate("click");
